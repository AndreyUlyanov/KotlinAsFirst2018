...
fun square(notation: String): Square = if (notation.length != 2 || notation[0] !in 'a'..'h' || notation[1] !in '1'..'8')
    ...
    when (notation[0]) {
        'a' -> Square(1, notation[1] - '0')
        'b' -> Square(2, notation[1] - '0')
        'c' -> Square(3, notation[1] - '0')
        'd' -> Square(4, notation[1] - '0')
        'e' -> Square(5, notation[1] - '0')
        'f' -> Square(6, notation[1] - '0')
        'g' -> Square(7, notation[1] - '0')
        else -> Square(8, notation[1] - '0')
    }
...
fun bishopMoveNumber(start: Square, end: Square): Int = when {
    ...
    abs(start.column - end.column) % 2 == abs(end.row - start.row) % 2 -> 2
    else -> -1
}
...
fun bishopTrajectory(start: Square, end: Square): List<Square> {
    val square1 = Square((start.column + start.row + end.column - end.row) / 2,
            (start.column + start.row - end.column + end.row) / 2)
    val square2 = Square((start.column - start.row + end.column + end.row) / 2,
            (start.row - start.column + end.column + end.row) / 2)
    return when (bishopMoveNumber(start, end)) {
        -1 -> listOf()
        0 -> listOf(start)
        1 -> listOf(start, end)
        else -> if (square1.inside()) listOf(start, square1, end)
        else listOf(start, square2, end)
    }
}
